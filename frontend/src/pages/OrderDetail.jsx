...
export default function OutletDetail() {
  ...
  // Clear the toast on its own so it doesn't sit over the menu
  useEffect(() => {
    if (!toast) return;
  ...
  async function addToBasket(dish) {
    ...
    try {
      ...
      setBasket(result);
      setToast({ kind: 'ok', text: `Added ${dish.title}` });
    } catch (err) {
    ...
    return (
      <Layout>
        <div className="mx-auto max-w-6xl px-5 py-20 text-center">
          ...
          <Link to="/" className="rounded-xl bg-sage-500 px-5 py-2.5 text-sm font-semibold text-white">
            Back to restaurants
          </Link>
        </div>
      </Layout>
  ...
  // The basket can now hold several restaurants, so these are totals
  // across all of them — not just this one.
  const basketCount = basket?.itemCount || 0;
  const basketTotal = basket?.total || 0;
  const restaurantCount = basket?.groups?.length || 0;
  ...
  return (
    <Layout>
      ...
      {/* Category nav sticks under the header on long menus */}
      {categories.length > 1 && (
              ...
              {grouped[category].map((d) => (
                ...
                >
                  ...
                  <div className="flex flex-col items-end justify-between">
                    <Tile name={d.title} className="h-16 w-16" rounded="rounded-xl" />
                    <button
          ...
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-5 py-3.5">
            <div>
              ...
              <p className="text-sm text-muted">
                Rs {basketTotal}
                {restaurantCount > 1 && ` · ${restaurantCount} restaurants`}
              </p>
            </div>
            <Link
              to="/basket"
